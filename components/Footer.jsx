...
const footerLinks = [
  {
    title: "Services",
    links: [
      { name: "Corporate cabs", href: "/#services" },
      { name: "Airport Transfers", href: "/#services" },
      { name: "Employee Transport", href: "/#services" },
  ...
  },
  {
    title: "Company",
    links: [
      { name: "Premium Car Rentals", href: "/services" },
      { name: "Bus & Shuttle", href: "/services" },
      { name: "Wedding & Events", href: "/services" },
      { name: "Airport & Outsation", href: "/services" },
    ],
  }
];
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-12 py-16">
          {footerLinks.map((section, idx) => (
            <div key={idx} className="space-y-6">
              <h4 className="font-black uppercase tracking-[3px] text-[10px] text-orange-500">
                {section.title}
              </h4>
              <ul className="space-y-4">
                {section.links.map((link, lIdx) => (
                  <li key={lIdx}>
                    <a href={link.href} className="text-slate-400 hover:text-white text-sm font-bold transition-colors">
                      {link.name}
                    </a>
                  </li>
                ))}
